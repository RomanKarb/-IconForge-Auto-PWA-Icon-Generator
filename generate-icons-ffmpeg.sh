...
# ====================================================================
# PWA Icon Generator Script for Linux/Mac (Bash + FFmpeg)
...
#   2. Запустите: chmod +x generate-icons-ffmpeg.sh && ./generate-icons-ffmpeg.sh
...
print_header() {
    echo -e "\n${CYAN}╔════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║         PWA ICON GENERATOR v1.0 (Bash + FFmpeg)              ║${NC}"
...
# ====== ПРОВЕРКА FFMPEG ======
print_header
print_info "🔍 Проверяю FFmpeg..."

if ! command -v ffmpeg &> /dev/null; then
    print_error "FFmpeg не найден!"
    echo ""
    print_warning "Пожалуйста установите FFmpeg:"
    echo ""
    echo "На Mac: brew install ffmpeg"
    echo "На Linux: sudo apt-get install ffmpeg"
...
print_success "FFmpeg найден: $FFMPEG_VERSION"
...
generate_icon() {
    ...
    ffmpeg -i "$SOURCE_ICON" -vf "scale=$size:$size" "$output_path" -y 2>/dev/null
...
echo ""
print_info "📁 Иконки сохранены в папку: $OUTPUT_DIR"
print_info "📝 HTML header сохранен в: $HEADER_FILE"
