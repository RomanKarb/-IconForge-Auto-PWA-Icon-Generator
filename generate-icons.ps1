...
Write-Host "║         PWA ICON GENERATOR v1.0 (PowerShell)                 ║" -ForegroundColor $InfoColor
Write-Host "║    Генератор иконок для iOS, Android, Web и PWA               ║" -ForegroundColor $InfoColor
Write-Host "╚════════════════════════════════════════════════════════════════╝" -ForegroundColor $InfoColor
Write-Host "`n"

# ====== ПРОВЕРКА IMAGEMAGICK ======
Write-Host "🔍 Проверяю ImageMagick..." -ForegroundColor $InfoColor

try {
    $magick = Get-Command magick -ErrorAction Stop
    Write-Host "✓ ImageMagick найден: $($magick.Source)" -ForegroundColor $SuccessColor
} catch {
    Write-Host "❌ ОШИБКА: ImageMagick не найден!" -ForegroundColor $ErrorColor
    Write-Host "`nПожалуйста установите ImageMagick:" -ForegroundColor $WarningColor
    Write-Host "https://imagemagick.org/script/download.php#windows" -ForegroundColor $InfoColor
...
function Generate-Icon {
    ...
    & magick "$SourceIcon" -resize "$($Size)x$($Size)" "$OutputPath" 2>$null
...
Write-Host "   1. Скопируйте всю папку '$OutputDir' в папку вашего проекта" -ForegroundColor $InfoColor
Write-Host "   2. Откройте файл '$HeaderFile' и скопируйте HTML в index.html" -ForegroundColor $InfoColor
Write-Host "   3. Если используете Vite, убедитесь что publicDir = `"public`"" -ForegroundColor $InfoColor
Write-Host "   4. Запустите: npm run build" -ForegroundColor $InfoColor

Write-Host "`n🔗 Справка:" -ForegroundColor $InfoColor
Write-Host "   - Web иконки должны быть в public/icons/" -ForegroundColor $InfoColor
Write-Host "   - Apple иконки в public/icons/apple/" -ForegroundColor $InfoColor
Write-Host "   - Android иконки в public/icons/android/mipmap-*/" -ForegroundColor $InfoColor
Write-Host "   - favicon.ico должен быть в public/" -ForegroundColor $InfoColor
